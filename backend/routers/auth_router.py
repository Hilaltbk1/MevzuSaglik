...
def register_user(request: dict, db: Session = Depends(get_db)):
    ...
    # Her kullanıcı için demo olması adına arka planda yeni bir Tenant açabiliriz (veya default tenant kullanabiliriz)
    # Burada basit olması için "free" planında yeni bir tenant açıyoruz
    ...
    # Yeni session_uuid de dönelim ki frontend hemen sohbete başlayabilsin
...
def login_user(request: dict, db: Session = Depends(get_db)):
    ...
    # Başarılı girişte yeni bir session_uuid döndürelim
